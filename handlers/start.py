...
async def _show_or_edit_menu(message, context, lang, user_id, edit=False):
    ...
    # Telegram’s blue bottom-left menu label is controlled by the Telegram client.
    # We provide a real persistent Reply Keyboard button named «▶️ استارت» for a one-tap start.
    if not context.user_data.get("start_keyboard_sent"):
        await message.reply_text("▶️ برای باز کردن سریع پنل، از دکمه استارت استفاده کن.", reply_markup=start_reply_keyboard(lang))
...
async def language_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text("🌐 زبان با موفقیت تغییر کرد.\n\n" + t(lang, "main_menu"), reply_markup=_menu_markup(lang, query.from_user.id))
